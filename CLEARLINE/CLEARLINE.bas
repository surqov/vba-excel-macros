...
Private Function RepSymb(ByVal line_ As String, Optional dic_ As Variant, Optional check_doubles As Boolean) As String
    ...
    If (IsDict) Then
        ...
        objRegExp.Pattern = "[^a-zA-Z0-9‡-ˇ¿-ﬂ" & EscapedDic & "]*"
    Else
    'Using standart symbols ranges if there no dictonary got from user
        objRegExp.Pattern = "[^a-zA-Z0-9‡-ˇ¿-ﬂ]*"
    End If
...
Public Function ”¡–À»ÿÕ(ByVal Rng As Range, Optional dic_ As Variant, Optional check_doubles As Boolean) As Variant
    ”¡–À»ÿÕ = CLEARLINE(Rng, dic_, check_doubles)
End Function
